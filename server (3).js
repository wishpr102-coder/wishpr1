  ...
  try {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 120,
        system: getBotSystemPrompt(persona),
        messages,
      }),
    });
    const data = await res.json();
    return data.content?.[0]?.text || 'lol';
  } catch(e) {
    return 'haha yeah';
  }
}
...
function botJoinQueue(botUsername) {
  ...
  // Add bot fake socket to online map
  onlineSockets.set(session.fakeSocketId, botUsername);
  userSockets.set(botUsername, session.fakeSocketId);

  if (waitingQueue.length > 0) {
    // Match with a real waiting user
    const pid = waitingQueue.shift();
    const pu = onlineSockets.get(pid);
    if (!pu) { waitingQueue.push(session.fakeSocketId); return; }

    const room = 'stranger_' + crypto.randomBytes(4).toString('hex');
    activePairs.set(session.fakeSocketId, pid);
    activePairs.set(pid, session.fakeSocketId);
    session.room = room;
    session.partnerSocketId = pid;
    session.history = [];
    ...
    io.sockets.sockets.get(pid)?.emit('matched', { partner: botProfile, room });
    ...
    // No one waiting — bot waits in queue
...
function handleBotMessage(fromSocketId, text, room) {
  // Find which bot is paired with this socket
  const botSocketId = activePairs.get(fromSocketId);
  if (!botSocketId || !botSocketId.startsWith('bot_')) return false;
...
function scheduleBotRefresh(botUsername) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => botJoinQueue(botUsername), Math.random() * 10000 + 5000);
  }, timeout);
}
