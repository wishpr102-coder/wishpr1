...
// Models
let AccountModel, SessionModel, RoomModel, DMRoomModel, AdminLogModel;
if (MONGO_URI) {
  AccountModel  = mongoose.model('Account',  AccountSchema);
  SessionModel  = mongoose.model('Session',  SessionSchema);
  RoomModel     = mongoose.model('Room',     RoomSchema);
  DMRoomModel   = mongoose.model('DMRoom',   DMRoomSchema);
...
const DB = {
  async getAccount(username) {
    if (!MONGO_URI) return memAccounts.get(username) || null;
    return AccountModel.findOne({ username }).lean();
  },
  async saveAccount(account) {
    if (!MONGO_URI) { memAccounts.set(account.username, account); return account; }
  ...
  async accountExists(username) {
    if (!MONGO_URI) return memAccounts.has(username);
    return !!(await AccountModel.exists({ username }));
  },
  async countAccounts() {
    if (!MONGO_URI) return memAccounts.size;
    return AccountModel.countDocuments();
  },
  async getAllAccounts() {
    if (!MONGO_URI) return Array.from(memAccounts.values());
    return AccountModel.find().lean();
  },
  async searchAccounts(q) {
    if (!MONGO_URI) return Array.from(memAccounts.values()).filter(a => !a.isBanned && (a.username.includes(q) || a.displayName.toLowerCase().includes(q)));
    return AccountModel.find({ isBanned: false, $or: [{ username: { $regex: q, $options: 'i' } }, { displayName: { $regex: q, $options: 'i' } }] }).limit(10).lean();
  },
  async getSession(token) {
    if (!MONGO_URI) return memSessions.get(token) || null;
    const s = await SessionModel.findOne({ token }).lean();
    return s ? s.username : null;
  },
  async saveSession(token, username) {
    if (!MONGO_URI) { memSessions.set(token, username); return; }
    await SessionModel.create({ token, username });
  },
  async deleteSession(token) {
    if (!MONGO_URI) { memSessions.delete(token); return; }
    await SessionModel.deleteOne({ token });
  },
  async getRoom(id) {
    if (!MONGO_URI) return memRooms.get(id) || null;
    return RoomModel.findOne({ id }).lean();
  },
  async saveRoom(room) {
    if (!MONGO_URI) { memRooms.set(room.id, room); return room; }
    return RoomModel.findOneAndUpdate({ id: room.id }, room, { upsert: true, new: true, lean: true });
  },
  async deleteRoom(id) {
    if (!MONGO_URI) { memRooms.delete(id); return; }
    await RoomModel.deleteOne({ id });
  },
  async getAllRooms() {
    if (!MONGO_URI) return Array.from(memRooms.values());
    return RoomModel.find().lean();
  },
  async getDMRoom(id) {
    if (!MONGO_URI) return memDMRooms.get(id) || null;
    return DMRoomModel.findOne({ id }).lean();
  },
  async saveDMRoom(room) {
    if (!MONGO_URI) { memDMRooms.set(room.id, room); return room; }
    return DMRoomModel.findOneAndUpdate({ id: room.id }, room, { upsert: true, new: true, lean: true });
  },
  async getDMRoomsForUser(username) {
    if (!MONGO_URI) return Array.from(memDMRooms.values()).filter(r => r.users.includes(username));
    return DMRoomModel.find({ users: username }).lean();
  },
  async logAdmin(by, action, target, detail) {
    if (!MONGO_URI) { memAdminLogs.push({ by, action, target, detail, timestamp: Date.now() }); if (memAdminLogs.length > 1000) memAdminLogs.splice(0, memAdminLogs.length - 1000); return; }
    await AdminLogModel.create({ by, action, target, detail });
  },
  async getAdminLogs() {
    if (!MONGO_URI) return memAdminLogs.slice(-200).reverse();
